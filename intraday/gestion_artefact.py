...
if artifact_id is None:
    raise Exception(f"Artifact nommé '{ARTIFACT_NAME}' non trouvé dans la run {run_id}")
...
# 4. Extraire et enregistrer localement
with zipfile.ZipFile(io.BytesIO(zip_resp.content)) as z:
    z.extractall(f".//{ARTIFACT_NAME}")

print(f"Artifact '{ARTIFACT_NAME}' extrait dans ./artifacts/{ARTIFACT_NAME}")
